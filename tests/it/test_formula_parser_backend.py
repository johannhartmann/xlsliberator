...
def test_backend_formula_parser_hook(skip_if_no_lo: None) -> None:
    """Backend formula parser should use UNO FormulaParser when pyuno is available."""
    backends = discover_backends()
    if not backends:
        pytest.skip("No office backend discovered")

    result = backends[0].parse_formula_text("=SUM(1;2;3)", sheet_name="Sheet1")
    if result.details.get("target_parser") != "uno_formula_parser":
        pytest.skip(
    ...
    assert invalid_result.details["target_parser"] == "uno_formula_parser"
